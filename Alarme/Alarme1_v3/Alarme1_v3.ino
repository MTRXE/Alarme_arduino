...
void AfficherMenu() {
  Serial.println("Menu :");
  if (etatAlarme == 0) {
      Serial.println("A : Activer l'alarme");
      Serial.println("C : Changer le code");
  } else if (etatAlarme == 1) {
      Serial.println("B : Désactiver l'alarme");
  }
  Serial.println("D : Quitter");
}
...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    if (Serial.available() > 0) {
      Serial.println("Erreur: Plusieurs caractères détectés. Veuillez entrer une seule commande.");
